...
export const useQueue = (payload: any) => {
  ...
  const hasDiscordId = useMemo(() => {
    return payload?.properties.some(
      (property: any) => property.type === "discord_id",
    )
  }, [payload])
  ...
  const joinQueue = useCallback(() => {
    ...
    queueIO.connect()
  }, [reset, start, hasDiscordId, payload, handleError])
  ...
  useEffect(() => {
    ...
    queueIO.on("error", handleServerError)

    return () => {
      queueIO.off("error", handleServerError)
    }
  }, [handleError])
