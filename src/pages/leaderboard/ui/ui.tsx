...
export const LeaderboardPage = () => {
    ...
    return (
        <Container display="flex" justifyContent="center" flexDirection="column" alignItems="center" gap="4" mt="2">
            <Text fontSize="3xl" color={textColor}>Leaderboard</Text>
...
export const RatingList = () => {
    return USERS.map((user, idx) => (
        <PlayerBox key={idx} id={idx + 1} username={user.username} rating={user.rating} role={user.role} />
    ));
};
...
export const PlayerBox = ({ id, username, rating, role }: PlayerProps) => {
    ...
    return (
        ...
        >
            <Box display="flex" alignItems="center" justifyContent="space-between" flexDirection="row" gap="16">
                <Box display="flex" flexDirection="row" alignItems="center" gap="2">
                    <Text color={textColor}>#{id}</Text>
                    <Text fontSize="18px" color={textColor}>{username}</Text>
                </Box>
                <AvatarGroup>
                    <Avatar size="xl" name={username} shape="rounded" src={`https://skin.vimeworld.com/helm/${username}.png`} />
                </AvatarGroup>
            </Box>
