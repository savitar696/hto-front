    ...
    <Flex flexDirection={"column"} gap={2} width={"100%"}>
      {players.map((player) => (
        <MenuRoot>
          ...
          <MenuContent>
            <MenuItem value="profile" onClick={() => navigate(`/profile/${player.name}`)}>Перейти на профиль</MenuItem>
            <MenuItem value="delete" color="fg.error" _hover={{ bg: "bg.error", color: "fg.error" }}>Пожаловаться</MenuItem>
          </MenuContent>
        </MenuRoot>
